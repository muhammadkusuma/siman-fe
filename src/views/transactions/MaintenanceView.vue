...
// import axios from 'axios'; // Uncomment axios agar tidak error

// --- STATE ---
const maintenances = ref([]);
const assets = ref([]); // Data aset untuk dropdown
...
const form = ref({
    id: null,
    asset_id: '',
    issue_date: new Date().toISOString().split('T')[0], // Default hari ini (YYYY-MM-DD)
    status: 'Pending',
    description: '',
    vendor_name: '',
    cost: 0,
...
const fetchData = async () => {
    ...
    try {
        // Ambil data Maintenance & Assets secara parallel
        const [resMaintenance, resAssets] = await Promise.all([
            axios.get(`${API_URL}/maintenances`, { headers: { 'Authorization': `Bearer ${token}` } }),
...
const handleSubmit = async () => {
    ...
    try {
        ...
        // Logic Create Only (Sesuai HTML asli)
        await axios.post(`${API_URL}/maintenances`, payload, config);
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="p-4 border-b border-gray-100 bg-gray-50">
                <div class="relative w-full sm:w-64">
                    <input v-model="searchQuery" type="text" placeholder="Cari aset, vendor..."
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500">
                    ...
                    <tbody>
                        ...
                        <tr v-else v-for="item in filteredMaintenances" :key="item.id"
                            class="border-b hover:bg-gray-50 transition">
                            <td class="px-6 py-4 whitespace-nowrap text-gray-600 font-mono text-xs">
                                {{ formatDate(item.issue_date) }}
                            </td>
                            <td class="px-6 py-4">
                                <div class="font-bold text-gray-800">{{ item.asset ? item.asset.name : 'Unknown' }}
                                </div>
                                <div class="text-xs text-gray-500">{{ item.asset ? item.asset.inventory_code : '-' }}
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="text-xs text-gray-500 mt-1">
                                    <i class="fa-solid fa-store mr-1"></i> {{ item.vendor_name || '-' }}
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span v-if="item.status === 'Pending'"
                                    class="bg-gray-100 text-gray-800 px-2.5 py-0.5 rounded-full text-xs font-medium border border-gray-200">Pending</span>
                                <span v-else-if="item.status === 'Proses'"
                                    class="bg-yellow-100 text-yellow-800 px-2.5 py-0.5 rounded-full text-xs font-medium border border-yellow-200"><i
                                        class="fa-solid fa-clock mr-1"></i> Proses</span>
                                <span v-else-if="item.status === 'Selesai'"
                                    class="bg-green-100 text-green-800 px-2.5 py-0.5 rounded-full text-xs font-medium border border-green-200"><i
                                        class="fa-solid fa-check mr-1"></i> Selesai</span>
                            </td>
        ...
        <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
            <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col animate-fade-in-up">
                ...
                <div class="px-6 py-4 border-b bg-gray-50 flex justify-between items-center rounded-t-xl shrink-0">
                    <h3 class="text-lg font-bold text-gray-900">Form Lapor Perbaikan</h3>
                ...
                <form @submit.prevent="handleSubmit" class="p-6 space-y-6 overflow-y-auto">
                    ...
                    <div
                        class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-yellow-50 p-4 rounded-lg border border-yellow-100">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Lapor <span
                                    class="text-red-500">*</span></label>
                            <input v-model="form.issue_date" type="date" required
                                class="block w-full border border-gray-300 rounded-lg py-2 px-3 outline-none bg-white">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Status Pengerjaan <span
                                    class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Deskripsi Kerusakan <span
                                class="text-red-500">*</span></label>
                    ...
                    <div class="border-t pt-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input v-model="form.vendor_name" type="text"
                                    class="block w-full border border-gray-300 rounded-lg py-2 px-3 text-sm outline-none"
                        ...
                        <div>
                            ...
                            <textarea v-model="form.action_taken" rows="2"
                                class="block w-full border border-gray-300 rounded-lg py-2 px-3 text-sm outline-none placeholder-gray-400"
                                placeholder="Solusi yang dilakukan..."></textarea>
                        </div>
                    </div>
