...
const form = ref({
    ...
    photo: null,
    photo_preview: null // URL untuk preview (bisa dari backend atau local blob)
});

const API_URL = 'http://localhost:3000/api';
const BASE_URL = 'http://localhost:3000'; // Base URL untuk akses file statis
...
const handleSubmit = async () => {
    ...
    try {
        ...
        // Hanya append photo jika user memilih file baru
...
const openModal = (asset = null) => {
    if (asset) {
        ...
        form.value = {
            ...
            photo: null, // Reset input file fisik

            // Set preview dari path database (jika ada)
            // Gabungkan BASE_URL + path dari database (misal: uploads/assets/foto.jpg)
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="p-4 border-b border-gray-100 bg-gray-50">
                <div class="relative w-full sm:w-72">
                    <input v-model="searchQuery" type="text" placeholder="Cari nama, kode, atau NUP..."
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500">
                    ...
                    <tbody>
                        ...
                        <tr v-else v-for="item in filteredAssets" :key="item.id"
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        ...
                                        <img v-if="item.photo_path" :src="`${BASE_URL}/${item.photo_path}`"
                                            class="w-full h-full object-cover">
                                        <i v-else class="fa-solid fa-image text-gray-300 text-lg"></i>
                                    </div>
                                    <div>
                                        <div class="font-bold text-gray-900">{{ item.name }}</div>
                                        <div class="text-xs text-gray-500 font-mono">{{ item.inventory_code }} | NUP: {{
                                            item.nup }}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="text-xs text-gray-500">{{ item.department ? item.department.code : '-' }}
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span
                                    :class="{ 'bg-green-100 text-green-800': item.condition_status === 'Baik', 'bg-red-100 text-red-800': item.condition_status === 'Rusak', 'bg-yellow-100 text-yellow-800': item.operational_status === 'Dalam Perbaikan' }"
                                    class="px-2.5 py-0.5 rounded-full text-xs font-medium border border-opacity-20">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center space-x-2">
                                    <button @click="openModal(item)"
                                        class="p-1.5 text-yellow-500 hover:bg-yellow-50 rounded transition"><i
                                            class="fa-solid fa-pen"></i></button>
                                    <button @click="deleteAsset(item.id)"
                                        class="p-1.5 text-red-500 hover:bg-red-50 rounded transition"><i
        ...
        <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
            <div class="bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-fade-in-up">
                ...
                <div class="px-6 py-4 border-b bg-gray-50 flex justify-between items-center shrink-0">
                    <h3 class="text-lg font-bold text-gray-900">{{ isEditing ? 'Edit Data Aset' : 'Tambah Aset Baru' }}
                    </h3>
                ...
                <form @submit.prevent="handleSubmit" class="p-6 overflow-y-auto">
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="md:col-span-2">
                                <label class="block text-xs font-medium text-gray-700 mb-1">Nama Aset <span
                                        class="text-red-500">*</span></label>
                                <input v-model="form.name" type="text" required
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Kategori <span
                                        class="text-red-500">*</span></label>
                                <select v-model="form.asset_category_id" required
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white outline-none">
                                    <option value="" disabled>Pilih Kategori</option>
                                    <option v-for="c in categories" :key="c.id" :value="c.id">{{ c.name }} ({{
                                        c.kode_barang }})</option>
                                </select>
                            </div>
                            <div class="grid grid-cols-2 gap-2">
                                <div><label class="block text-xs font-medium text-gray-700 mb-1">Kode
                                        Inventaris</label><input v-model="form.inventory_code" type="text"
                                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"></div>
                                <div><label class="block text-xs font-medium text-gray-700 mb-1">NUP</label><input
                                        v-model="form.nup" type="number"
                                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div><label class="block text-xs font-medium text-gray-700 mb-1">Merk</label><input
                                    v-model="form.brand"
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"></div>
                            <div><label class="block text-xs font-medium text-gray-700 mb-1">Model</label><input
                                    v-model="form.model"
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"></div>
                            <div><label class="block text-xs font-medium text-gray-700 mb-1">Serial Number</label><input
                    ...
                    <div class="mb-6 bg-blue-50 p-4 rounded-lg border border-blue-100">
                        <h4 class="text-sm font-bold text-blue-800 mb-3">Lokasi Penempatan</h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div><label class="block text-xs font-medium text-gray-700 mb-1">Unit Pemilik</label><select
                                    v-model="form.department_id" required
                                    class="w-full border border-blue-300 rounded-lg px-3 py-2 text-sm bg-white">
                                    <option v-for="d in departments" :key="d.id" :value="d.id">{{ d.name }}</option>
                                </select></div>
                            <div><label class="block text-xs font-medium text-gray-700 mb-1">Gedung</label><select
                                    v-model="form.building_id" @change="handleBuildingChange"
                                    class="w-full border border-blue-300 rounded-lg px-3 py-2 text-sm bg-white">
                                    <option value="">- Pilih -</option>
                                    <option v-for="b in buildings" :key="b.id" :value="b.id">{{ b.name }}</option>
                                </select></div>
                            <div class="md:col-span-2"><label
                                    class="block text-xs font-medium text-gray-700 mb-1">Ruangan</label><select
                                    v-model="form.room_id" :disabled="!form.building_id" required
                                    class="w-full border border-blue-300 rounded-lg px-3 py-2 text-sm bg-white disabled:bg-gray-200">
                                    <option v-for="r in availableRooms" :key="r.id" :value="r.id">{{ r.name }} ({{
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="space-y-4">
                                <div><label class="block text-xs font-medium text-gray-700 mb-1">Kondisi</label><select
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    <div><label class="block text-xs font-medium text-gray-700 mb-1">Tgl Perolehan <span
                                                class="text-red-500">*</span></label><input
                                            v-model="form.acquisition_date" type="date" required
                                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"></div>
                                    <div><label class="block text-xs font-medium text-gray-700 mb-1">Harga</label><input
                        ...
                        <div>
                            <h4 class="text-sm font-bold text-gray-800 mb-3 border-b pb-1">Foto Aset</h4>
                            <div
                                class="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:bg-gray-50 transition relative group h-48 flex flex-col justify-center items-center">

                                <input type="file" @change="handleFileChange" accept="image/*"
                                    class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">

                                <div v-if="form.photo_preview" class="w-full h-full absolute inset-0 p-2">
                                    <img :src="form.photo_preview"
                                        class="w-full h-full object-contain rounded bg-gray-100">
                                    <div class="">
                                        <p
                                            class="text-white opacity-0 group-hover:opacity-100 text-xs font-bold bg-black bg-opacity-50 px-2 py-1 rounded">
                                            Klik untuk Ganti</p>
                                    </div>
                                </div>
                                ...
                                <div v-else class="text-gray-400">
                                    ...
                                    <p class="text-sm font-medium">Klik atau drop file disini</p>
                                    <p class="text-[10px]">JPG/PNG maks 2MB</p>
                                </div>
                            </div>
                        </div>
                    </div>
