...
// Import komponen Custom Select yang baru dibuat
...
// Handle Perubahan Gedung di Dropdown Baru
const handleBuildingChange = (buildingId) => {
    form.value.room_id = ''; // Reset ruangan
    
    // Cari building object berdasarkan ID yang dipilih
    const selectedBuilding = buildings.value.find(b => b.id === buildingId);
...
const openModal = (asset = null) => {
    if (asset) {
        ...
        availableRooms.value = [];
        form.value = {
            id: null, name: '', asset_category_id: '', inventory_code: '', nup: '',
            brand: '', model: '', serial_number: '', department_id: '', building_id: '',
            room_id: '', acquisition_date: '', price: 0, condition_status: 'Baik',
            photo: null, photo_preview: null
        };
    }
                ...
                <table class="w-full text-sm text-left text-gray-500">
                    ...
                    <tbody>
                        ...
                        <tr v-else v-for="item in filteredAssets" :key="item.id" class="border-b hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                <span :class="{ 'bg-green-100 text-green-800': item.condition_status === 'Baik', 'bg-red-100 text-red-800': item.condition_status === 'Rusak', 'bg-yellow-100 text-yellow-800': item.operational_status === 'Dalam Perbaikan' }" class="px-2.5 py-0.5 rounded-full text-xs font-medium border border-opacity-20">
                ...
                <form @submit.prevent="handleSubmit" class="p-6 overflow-y-auto">
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="md:col-span-2">
                                ...
                                <input v-model="form.name" type="text" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <div><label class="block text-xs font-medium text-gray-700 mb-1">Kode Inventaris</label><input v-model="form.inventory_code" type="text" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"></div>
                                <div><label class="block text-xs font-medium text-gray-700 mb-1">NUP</label><input v-model="form.nup" type="number" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"></div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <SearchableSelect 
                                    ...
                                    @change="handleBuildingChange(form.building_id)" 
                                />
                            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div class="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:bg-gray-50 transition relative group h-48 flex flex-col justify-center items-center">
                                ...
                                <div v-if="form.photo_preview" class="w-full h-full absolute inset-0 p-2">
                                    <img :src="form.photo_preview" class="w-full h-full object-contain rounded bg-gray-100">
                                </div>
                    ...
                    <div class="flex justify-end gap-3 pt-6 mt-6 border-t">
                        ...
                        <button type="submit" :disabled="isSubmitting" class="px-6 py-2 bg-blue-600 text-white rounded-lg">{{ isSubmitting ? 'Menyimpan...' : 'Simpan Aset' }}</button>
                    </div>
