...
const buildings = ref([]); // Berisi data gedung beserta ruangannya
const availableRooms = ref([]); // Opsi ruangan berdasarkan gedung yang dipilih

const isLoading = ref(false);
const isSubmitting = ref(false);
const isModalOpen = ref(false);
const isEditing = ref(false); // Mode Edit atau Create
...
const form = ref({
    ...
    serial_number: '',
    department_id: '',
    building_id: '', // Helper state untuk filter room
    room_id: '',
    acquisition_date: '',
    price: 0,
    ...
    photo_preview: null // URL untuk preview gambar
});

const API_URL = 'http://localhost:3000/api';
const BASE_URL = 'http://localhost:3000'; // Untuk load gambar
const token = localStorage.getItem('token');

// --- API ACTIONS ---

// 1. Fetch All Data (Aset & Master Data)
...
// 2. Submit Form (Create / Update dengan Upload File)
const handleSubmit = async () => {
    ...
    try {
        ...
        // Handle Date (Convert to ISO String if exists)
        ...
        // Handle File (Hanya jika user memilih file baru)
...
const deleteAsset = async (id) => {
    if (!confirm('Yakin ingin menghapus aset ini? Data tidak bisa dikembalikan.')) return;
    try {
...
const handleBuildingChange = () => {
    form.value.room_id = ''; // Reset pilihan ruangan
...
const handleFileChange = (event) => {
    ...
    if (file) {
        ...
        form.value.photo_preview = URL.createObjectURL(file); // Preview lokal
    }
};
...
const openModal = (asset = null) => {
    if (asset) {
        ...
        // Cari Gedung ID berdasarkan Room ID aset (karena di DB aset nyimpan room_id saja)
        let buildingId = '';
        const foundBuilding = buildings.value.find(b => b.rooms?.some(r => r.id === asset.room_id));
        if (foundBuilding) {
            buildingId = foundBuilding.id;
            availableRooms.value = foundBuilding.rooms; // Isi opsi ruangan
        } else {
            availableRooms.value = [];
        }
                ...
                <table class="w-full text-sm text-left text-gray-500">
                    ...
                    <tbody>
                        ...
                        <tr v-else v-for="item in filteredAssets" :key="item.id"
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded bg-gray-200 overflow-hidden shrink-0">
                                        ...
                                        <div v-else
                                            class="w-full h-full flex items-center justify-center text-gray-400"><i
                            ...
                            <td class="px-6 py-4">
                                <span :class="{
                                    'bg-green-100 text-green-800': item.condition_status === 'Baik',
                                    'bg-red-100 text-red-800': item.condition_status === 'Rusak' || item.condition_status === 'Rusak Berat',
                                    'bg-yellow-100 text-yellow-800': item.operational_status === 'Dalam Perbaikan' || item.condition_status === 'Rusak Ringan'
                                }" class="px-2.5 py-0.5 rounded-full text-xs font-medium border border-opacity-20">
                                    {{ item.operational_status === 'Dalam Perbaikan' ? 'Perbaikan' :
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center space-x-2">
                                    ...
                                    <button @click="deleteAsset(item.id)"
                                        class="p-1.5 text-red-500 hover:bg-red-50 rounded transition" title="Hapus">
            ...
            <div class="bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-fade-in-up">
                ...
                <div class="px-6 py-4 border-b bg-gray-50 flex justify-between items-center shrink-0">
                    <h3 class="text-lg font-bold text-gray-900">
                        {{ isEditing ? 'Edit Data Aset' : 'Tambah Aset Baru' }}
                    </h3>
                ...
                <form @submit.prevent="handleSubmit" class="p-6 overflow-y-auto">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <input v-model="form.inventory_code" type="text"
                                ...
                                <div>
                                    ...
                                    <input v-model="form.nup" type="number"
                                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm outline-none">
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Merk / Brand</label>
                                <input v-model="form.brand" type="text"
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm outline-none">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Model / Tipe</label>
                    ...
                    <div class="mb-6 bg-blue-50 p-4 rounded-lg border border-blue-100">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Unit / Prodi Pemilik <span
                                        class="text-red-500">*</span></label>
                                <select v-model="form.department_id" required
                                    class="w-full border border-blue-300 rounded-lg px-3 py-2 text-sm bg-white outline-none">
                            ...
                            <div>
                                ...
                                <select v-model="form.building_id" @change="handleBuildingChange"
                                    class="w-full border border-blue-300 rounded-lg px-3 py-2 text-sm bg-white outline-none">
                                    <option value="">- Pilih Gedung -</option>
                                    <option v-for="b in buildings" :key="b.id" :value="b.id">{{ b.name }} ({{ b.code }})
                                    </option>
                                </select>
                            </div>
                            <div class="md:col-span-2">
                                <label class="block text-xs font-medium text-gray-700 mb-1">Ruangan <span
                                        class="text-red-500">*</span></label>
                                <select v-model="form.room_id" :disabled="!form.building_id" required
                                    class="w-full border border-blue-300 rounded-lg px-3 py-2 text-sm bg-white outline-none disabled:bg-gray-200 disabled:cursor-not-allowed">
                                    <option value="" disabled>Pilih Gedung Terlebih Dahulu</option>
                                    <option v-for="r in availableRooms" :key="r.id" :value="r.id">{{ r.name }} - Lt.{{
                                        r.floor }} ({{ r.room_number }})</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1">Kondisi Aset</label>
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    <div>
                                        <label class="block text-xs font-medium text-gray-700 mb-1">Tgl
                                            Perolehan</label>
                        ...
                        <div>
                            ...
                            <div
                                ...
                                <div v-if="form.photo_preview" class="relative">
                                    <img :src="form.photo_preview" class="h-32 mx-auto object-cover rounded shadow-sm">
                                    <p class="text-xs text-blue-600 mt-2">Klik untuk ganti foto</p>
                                </div>
                                <div v-else class="py-8">
                                    <i class="fa-solid fa-cloud-arrow-up text-3xl text-gray-400 mb-2"></i>
                                    <p class="text-sm text-gray-500">Klik atau drop file disini</p>
